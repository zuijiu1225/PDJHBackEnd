<template>
  <div class="body" v-loading="BodayLoading">
    ...
    <el-row class="row1daoji">
      <!-- <el-col :span="12" style="position: absolute; bottom: 0; right: 0">
    ...
    <el-row class="row2">
      ...
      <el-col :span="10" :xs="24" :sm="16" class="row2_col2">
        <div class="row2_keyboard">
          <div class="row2_con">
            <el-row v-for="i in Math.ceil(List.length / 3)" :key="i">
              <el-col
                class="row2_col"
                :span="3"
                ...
                :key="j"
              >
                <el-button @click="ClickInput(j)"
                  ><span class="ButtonText">{{ j }}</span></el-button
                >
              </el-col>
            </el-row>
            <el-row class="row2_rowfh">
              <el-col class="row2_colfh">
                <el-button
                  icon="el-icon-back"
                  style="height: 50px"
                  @click="Clickfh()"
    ...
    <el-dialog
      title="患者登记"
      :visible.sync="dialogRegsiterVisible"
      :before-close="Clickquxiao"
    >
      <el-row>
        <el-form
          :model="SignIn"
        ...
        >
            ...
            <el-col :span="12">
              <el-form-item label="流水号:" prop="PatientID">
                <!-- <el-input
                  ...
                  v-model=""
                ></el-input> -->
                <!-- <span>{{ GetCreditCardDataInputres[0].PatientID}}</span> -->
                <span>{{
                  GetCreditCardDataInputres[0] != null &&
                  GetCreditCardDataInputres[0] != undefined
                    ? GetCreditCardDataInputres[0].PatientID
                    : null
                }}</span>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="患者姓名:" prop="PatientName">
                <!-- <el-input
                  ...
                  element-loading-spinner="el-icon-loading"
                  element-loading-background="rgba(0, 0, 0, 0.8)"
                  v-model="GetCreditCardDataInputres[0].PatientName"
                ></el-input>
              <span>{{GetCreditCardDataInputres[0].PatientName}}</span> -->
                <span v-if="GetCreditCardDataInputres != []">{{
                  GetCreditCardDataInputres[0] != null &&
                  GetCreditCardDataInputres[0] != undefined
                    ? GetCreditCardDataInputres[0].PatientName
                    : null
                }}</span>
              </el-form-item>
            </el-col>
          ...
          <el-row>
            <el-checkbox-group v-model="checkList">
              <el-card
                class="box-card"
                v-for="(i, index) in GetCreditCardDataInputres"
                :key="index"
              >
                <el-checkbox :label="i.Item" style="font-size: 24px">
                </el-checkbox>
              </el-card>
            </el-checkbox-group>
          </el-row>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="Clickquxiao()">取 消</el-button>
        <el-button type="primary" @click="ClickItmes()">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    GetCreditCardBtnData() {
      ...
      GetCreditCardBtnData(this.GetCreditCardDataInput)
        .then((res) => {
          ...
          if (res && res.result == true) {
            ...
            this.GetCreditCardDataInputres = data;
            if (this.GetCreditCardDataInputres.length > 1) {
              this.dialogRegsiterVisible = true; //获取数据后，判断项目是否需要显示弹窗
            } else {
              this.VerifyPatientSign(this.GetCreditCardDataInputres[0]);
            }
    ...
    //判断是否签到
    VerifyPatientSign(element) {
      console.log("VerifyPatientSignelement", element);
      this.BodayLoading = true;
      var data = {
        ...
        .then((res) => {
          if (res && res.result == true) {
            ...
            if (res.data.IsSign == true) {
              ...
              this.ConfirmPatientBtnSign(element);
            }
          } else {
            this.SerialNumber = "";
            this.ShowTip = res.msg;
    ...
    ClickItmes() {
      //显示弹窗，后确定
      console.log(
        "this.GetCreditCardDataInputres",
        this.GetCreditCardDataInputres
      );
      var VerifyPatientSignList = [];
      for (var i = 0; i < this.checkList.length; i++) {
        for (var j = 0; j < this.GetCreditCardDataInputres.length; j++) {
          if (this.GetCreditCardDataInputres[j].Item == this.checkList[i]) {
            VerifyPatientSignList.push(this.GetCreditCardDataInputres[j]);
          }
        }
      }
      console.log("VerifyPatientSignList", VerifyPatientSignList);
      for (var i = 0; i < VerifyPatientSignList.length; i++) {
        console.log("VerifyPatientSignList[i]", VerifyPatientSignList[i]);
        this.VerifyPatientSign(VerifyPatientSignList[i]);
      }
    },
    Clickfh() {
      ...
      this.$router.go(-1);
      // this.$router.replace({ path: "/Singin/" + this.ProtCode });
    },
    TimingTip() {
      setTimeout(() => {
  ...
  mounted() {
    ...
    this.GetCreditCardDataInput.BtnID = localStorage.getItem("BtnID");
    this.BtnID = localStorage.getItem("BtnID");
  },
};
</script>
